...
import Common_methods.Common_methods_Patch;
...
public class Patch_TC1 {
	...
	public static void orchestrator() throws IOException
	{
		...
		String baseUri = PatchRequestRepository.BaseURL();
		String resource = PatchRequestRepository.resource();
		String requestBody = PatchRequestRepository.Patch_Request_TC1();

		for (int i=0; i<5; i++)
		{	
			res_StatusCode=Common_methods_Patch.responseStatusCodeExtractor(baseUri, resource, requestBody);
			
			if (res_StatusCode == 200)
			{
				responseBody = Common_methods_Patch.responseBodyExtractor(baseUri,resource, requestBody);
